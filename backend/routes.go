...
// 	// 🟢 Public: GET semua produk
// 	addRoute(api, "GET", "", []string{}, GetAllProducts, db)

// 	// 🔐 Admin only: POST, PATCH, DELETE
// 	addRoute(api, "POST", "", []string{"admin"}, CreateProduct, db)
// 	addRoute(api, "PATCH", "/:id", []string{"admin"}, UpdateProduct, db)
// 	addRoute(api, "DELETE", "/:id", []string{"admin"}, DeleteProduct, db)
// }
...
//     // 2. Ambil gambar & varian untuk setiap produk (optimasi: gunakan goroutine jika banyak produk)
...
// func getBasicProducts(db *sql.DB) ([]ProductsBasicModel, error) {
//     rows, err := db.Query(`
//         SELECT 
...
// 	query := `
// 		INSERT INTO products (category_id, name, description, is_varians, is_discounted, discount_price, price, stock, created_at, updated_at) 
// 		VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`
...
// 		Stock         sql.NullInt64
// 	}
// 	err := db.QueryRow(`SELEC, is_varians, is_discounted, discount_price, price, stock, is_service FROM products WHERE id = ?`, idInt).
// 		Scan(&product.CategoryID, &product.IsVarians, &product.IsDiscounted, &product.DiscountPrice, &product.Price, &product.Stock)
// 	if err != nil {
