      ...
      <div class="list-group mb-4">
        @foreach ($items as $item)
          <div class="list-group-item d-flex justify-content-between align-items-start">
            <div class="d-flex">
              <img src="{{ $item['thumbnails'][0] ?? '#' }}" width="60" class="me-3" style="object-fit:cover; border-radius:8px;">
              <div>
                <div><strong>ID Produk: {{ $item['product_id'] }}</strong></div>
                <div>{{ $item['name'] }}</div>
                @if(isset($item['variants']) && count($item['variants']) > 0)
                  <div class="text-muted small">{{ $item['variants'][0]['name'] }}</div>
                @endif
                <div class="text-muted">Kuantitas: {{ $item['quantity'] }}</div>
              </div>
            </div>
            <div class="text-end">
